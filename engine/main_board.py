...
class MainBoard(object):
    ...
    def __init__(self, board_size: int = 3):
        ...
        self.steps = 0
        self.main_cell_value = [[0, 0, 0], [0, 0, 0], [0, 0, 0]]
    ...
    def get_oppo_coords(self):
        if self.is_finished:
            return []
        oppo_boards = []
        for row_index in range(0, self._board_size):
            for col_index in range(0, self._board_size):
                if self._board[row_index][col_index].is_finished and self._board[row_index][col_index].winner == Player.OPPONENT:
                    oppo_boards.append((row_index, col_index))
        return oppo_boards
    ...
    def get_self_coords(self):
        ...
        for row_index in range(0, self._board_size):
            for col_index in range(0, self._board_size):
                if self._board[row_index][col_index].is_finished and self._board[row_index][col_index].winner == Player.ME:
                    self_boards.append((row_index, col_index))
        return self_boards
    
    def get_tie_coords(self):
        if self.is_finished:
            return []
        tie_boards = []
        for row_index in range(0, self._board_size):
            for col_index in range(0, self._board_size):
                if self._board[row_index][col_index].is_finished and self._board[row_index][col_index].winner == Player.NONE:
                    tie_boards.append((row_index, col_index))
        return tie_boards
