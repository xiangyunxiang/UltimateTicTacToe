...
class SubBoard(object):
    ...
    def __init__(self, board_size: int = 3):
        ...
        self.oppo_coord = []
        self.self_coord = []
        self.sub_cell_value = [[0, 0, 0], [0, 0, 0], [0, 0, 0]]
        self.win_sub_cond = [
            ((0, 0), (0, 1), (0, 2)),
            ((1, 0), (1, 1), (1, 2)),
            ((2, 0), (2, 1), (2, 2)),
    ...
    def add_move(self, sub_board_coords: SubBoardCoords, player: Player) -> 'SubBoard':
        ...
        if player == Player.ME:
            updated_sub_board.self_coord.append((sub_board_coords.row, sub_board_coords.col))
        else:
            updated_sub_board.oppo_coord.append((sub_board_coords.row, sub_board_coords.col))
    ...
    def get_opponent_coords(self) -> [SubBoardCoords]:

        if self.is_finished:
            return []
        return self.oppo_coord

    def get_self_coords(self) -> [SubBoardCoords]:

        if self.is_finished:
            return []
        return self.self_coord
